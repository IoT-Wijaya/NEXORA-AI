...
// Menggunakan LCD 20x4 alamat 0x27
...
String getSignalStatus(int rssi) {
  if (WiFi.status() != WL_CONNECTED) return "Terputus";
  if (rssi >= -65) return "Kuat";
  else if (rssi >= -75) return "Sedang";
  else return "Lemah";
}
...
int getEstimatedSpeedMbps(int rssi) {
  ...
  if (rssi >= -55) return 72;      
  else if (rssi >= -65) return 54; 
  else if (rssi >= -75) return 24; 
  else if (rssi >= -85) return 6;  
  else return 1;                   
}
